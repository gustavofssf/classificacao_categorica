...
# Adiciona o src ao path para importações absolutas
sys.path.append(os.path.join(os.path.dirname(__file__), 'src'))

from classificacao_categorica.dataset import load_data, preprocess_categorical_features
...
def main():
    ...
    try:
        ...
        # SUBSTITUA pelo nome real do seu arquivo e coluna target
        df = load_data('dataset_exemplo.csv')
        X, y, encoders = preprocess_categorical_features(df, 'target')

        print("Dados carregados e pré-processados com sucesso!")

        # Train model
        model, accuracy, f1, X_test, y_test = train_model(X, y)
        ...
        model_2, accuracy_2, f1_2, X_test_2, y_test_2 = train_model(X, y)
